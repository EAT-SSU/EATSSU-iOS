...
// 사용 x
class WriteReviewViewController: BaseViewController, UINavigationControllerDelegate {
  // MARK: - Properties
  ...
  override func setButtonEvent() {
    updateReviewButton.addTarget(self, action: #selector(nextButtonTapped), for: .touchUpInside)
  }
  ...
  // 해당 함수는 선언되고 호출되는 코드가 없음. 최초 작성자가 확인 후 공유.
  func getGradeTag(grade: Int, tag: [String]) {
    self.grade = grade
    self.tag = tag
  }
}
// MARK: - UIImagePickerController Delegateb
