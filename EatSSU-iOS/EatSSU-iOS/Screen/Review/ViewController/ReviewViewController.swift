...
final class ReviewViewController: BaseViewController {
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    self.getReviewRate()
    self.getReviewList(type: type, menuId: menuID)
  }
  ...
  private func setTableView() {
    ...
    reviewTableView.register(
      ReviewRateViewCell.self, forCellReuseIdentifier: ReviewRateViewCell.identifier)
    reviewTableView.register(
      ReviewEmptyViewCell.self, forCellReuseIdentifier: ReviewEmptyViewCell.identifier)
        ...
        DispatchQueue.main.async { [self] in
          ...
          if menuIDList == nil {
            ...
            if menuIDList?.count == 1 {
              ...
              choiceMenuViewController.menuDataBind(
                menuList: menuNameList, idList: menuIDList ?? [])
              self.activityIndicatorView.stopAnimating()
              self.navigationController?.pushViewController(
                choiceMenuViewController, animated: true)
            }
          }
        }
      }
...
extension ReviewViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    case 0:
      let cell =
        tableView.dequeueReusableCell(withIdentifier: ReviewRateViewCell.identifier, for: indexPath)
        as? ReviewRateViewCell ?? ReviewRateViewCell()
      cell.selectionStyle = .none
      if self.type == "FIXED" {
        cell.fixMenuDataBind(
          data: self.fixedResponseData
      ...
      if reviewList.count == 0 {
        let cell =
          tableView.dequeueReusableCell(
            withIdentifier: ReviewEmptyViewCell.identifier, for: indexPath) as? ReviewEmptyViewCell
          ?? ReviewEmptyViewCell()
        ...
        cell.handler = { [weak self] in
          ...
          reviewList[indexPath.row].isWriter
            ? showFixOrDeleteAlert(data: reviewList[indexPath.row])
...
extension ReviewViewController {
  ...
  // 하단 리뷰 리스트 불러오는 API
  private func getReviewList(type: String, menuId: Int) {
