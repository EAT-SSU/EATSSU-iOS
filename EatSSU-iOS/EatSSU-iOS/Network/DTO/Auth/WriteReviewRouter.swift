...
extension WriteReviewRouter: TargetType, AccessTokenAuthorizable {
  ...
  var path: String {
    switch self {
    case .writeReview(param: _, image: _, let menuId):
  ...
  var task: Moya.Task {
    switch self {
    case .writeReview(let param, image: let imageList, menuId: _):
      let jsonData: Data
      var multipartFileList: MultipartFormData
      do {
        jsonData = try JSONEncoder().encode(param)
        let multipartFormData = MultipartFormBodyPart(
      ...
      if imageList.count != 0 {
        for fileData in imageList {
          if let unwrappedImage = fileData {
            let imageData =
              unwrappedImage.resize(newWidth: 300.adjusted).jpegData(compressionQuality: 0.3)
              ?? Data()
            let formData = MultipartFormBodyPart(
    ...
    case .uploadImage(let image):
      ...
      let imageData =
        unwrappedImage.resize(newWidth: 300.adjusted).jpegData(compressionQuality: 0.3) ?? Data()
      let formData = MultipartFormBodyPart(
        provider: .data(imageData),
        name: "image",
        fileName: "image.jpeg",
      ...
      multipartFileList = [formData]
      return .uploadMultipartFormData(multipartFileList)
    case .writeNewReview(let param, _):
